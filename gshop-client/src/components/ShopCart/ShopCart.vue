    ...
    <div class="shopcart">
      <div class="content">
        <div class="content-left" @click="toggleShow">
          ...
          <div class="price" :class="{highlight:cartFoodCount}">￥{{cartFoodPrice}}</div>
  ...
  export default {
    ...
    computed: {
      ...
      sendStartPrice() {
        ...
        if (cartFoodPrice === 0) {
          return `￥${minPrice}元起送`
        } else if (minPrice > cartFoodPrice) {
      ...
      enoughClass(){
        const {cartFoodPrice} = this
        const {minPrice} = this.info
        return minPrice > cartFoodPrice ? 'not-enough':'enough'
      },
      listShow() {
        ...
        /*在创建滚动对象时，click事件会重复触发，会创造多个scroll对象
        * 创建之前必须判断this.scroll对象是否存在，不存在在创建*/
        if (this.isShow) {
          console.log(1111);
          this.$nextTick(() => {
            if (!this.scroll) {
